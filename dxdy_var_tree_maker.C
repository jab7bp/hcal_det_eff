...
double dx_sig_mean, dx_sig_sigam;
...
Double_t theta_pq_p_out, dx_out, dy_out, W2_out, W2_elastics_out, Q2_out, nu_out;
Double_t hcale_out, ps_e_out, sh_e_out, ep_out, e_over_p_out;
	...
	//--------------------------------------------------------------
		...
		//fill work histograms
			...
			//TIGHT ELASTIC CUTS FOR WORK
				...
				if( (b_hcal_clusblk_ADC_cut) && (theta_pq_p < theta_pq_p_thresh) && (dx>dx_min || dx<dx_max) && (dy>dy_min || dy<dy_max) && (!global_cut_fail) && hit_on_HCal ){

					h_W2elastics->Fill( W2 );
					h_W2elasticsFull->Fill( W2 );
				}
				if( (b_hcal_clusblk_ADC_cut) && (theta_pq_p < (0.30*theta_pq_p_thresh)) && (dx>dx_min || dx<dx_max) && (dy>dy_min || dy<dy_max) && (!global_cut_fail) && hit_on_HCal ){
...
//-----------------------------------------------
	...
	if( fit_only ){
		...
		// h_W2elastics = static_cast<TH1D*>(histo_infile->Get("h_W2elastics"));
...
//---------------------------------------------
	...
	//----- Then we take the parameters from that and fix the Full Fit BG parametes to match
			...
			h_W2copy->Fit("tf1_W2_bg_rej_init", "RBM0+");
			...
			h_W2copy->Fit("tf1_W2_bg_rej_full", "RBM0+");
	//----------
		...
		//Fix parameters from the BG_reject fit
			...
			h_W2copy->Fit("tf1_W2_fullFit_rej", "RBM0+");
			...
			h_W2copy->Fit("tf1_W2_fullFit_rej", "RBM+");
	...
	//--------------------------------------
		...
		tf1_W2_elastics_init = new TF1("tf1_W2_elastics_init", fit_gaus, W2_fit_min, W2_fit_max, 3);
		...
		h_W2elastics_corr->Fit("tf1_W2_elastics_init", "RBM0+");
		...
		for( int param = 1; param < polN+2; param++ ){
			tf1_W2_interpolate->SetParName(param, Form("W2 BG Pol%i p%i (Interpol)", polN, param-1));
		}
		h_W2interpol->Fit("tf1_W2_interpolate", "RBM0+");
		tf1_W2_interpolate->GetParameters(&parInterpol[0]);
		tf1_W2_interpolate->GetParameters(&par[0]);

		if( fit_with_expo ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fit_expo, W2_fit_min, W2_fit_max, polNfit);					
		}

		if( polN == 3 ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fitPol3, W2_fit_min, W2_fit_max, polNfit);					
		}

		if( polN == 4 ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fitPol4, W2_fit_min, W2_fit_max, polNfit);					
		}

		if( polN == 5 ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fitPol5, W2_fit_min, W2_fit_max, polNfit);					
		}

		if( polN == 6 ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fitPol6, W2_fit_min, W2_fit_max, polNfit);					
		}

		if( polN == 8 ){
			tf1_W2_interpol_bg = new TF1("tf1_W2_interpol_bg", fitPol8, W2_fit_min, W2_fit_max, polNfit);					
		}

		tf1_W2_interpol_bg->SetNpx(nBins_fit_W2);

		for( int param = 0; param < polNfit; param++ ){
		...
		h_W2interpol->Fit("tf1_W2_interpol_bg", "RBM0+");

		// h_W2elastics_init->Draw("same");

		tf1_W2_interpolate_full = new TF1("tf1_W2_interpolate_full", fullFitFunction, W2_fit_min, W2_fit_max, polN+4);
		...
		for( int param = 3; param <= (polN + 4); param++ ){
			...
			tf1_W2_interpolate_full->SetParName(param, Form("Interpol pol%i p%i", polN, param-3));
		}
		...
		TF1 *tf1_W2_interpolate_gaus = new TF1("tf1_W2_interpolate_gaus", fit_gaus, 0, 1.4, 3);
		...
		h_W2_interpol_sig = new TH1D("h_W2_interpol_sig", "h_W2_interpol_sig", h_W2interpol->GetXaxis()->GetNbins(), 0.0, 1.4);
		...
		tp_W2_interpol->AddText(Form("Interpolated fit with pol%i", polN));
...
//------------------------------------------------------
		...
		//BG Pol3
			...
			tl_dy_min->SetLineColor(6);
			tl_dy_min->SetLineStyle(4);

			TLine *tl_dy_max = new TLine(dy_sig_max, -1.5, dy_sig_max, 1.5);
			tl_dy_max->SetLineColor(6);
...
//------------------------------------------------------
		...
		TH1D *h_dx_subrange_sig = (TH1D*)h_dx_subrange->Clone("h_dx_subrange_sig");


		double dx_sig_integral = h_dx_sig->Integral();
	...
	// 	sig_integral = h_dx_sig->Integral();
		...
		TPaveText *tp_dx = new TPaveText(0.60, 0.35, 0.90, 0.69, "NDCbr");
		tp_dx->AddText(Form("Total fit chi-sq: %.2f", tf1_dx_fullFit->GetChisquare()));
		tp_dx->AddText(Form("Estimated real dx count: %i", int(dx_sig_integral)));
		tp_dx->AddText("---------");
		tp_dx->AddText(Form("Estimated No Rej. W2 count: %i", int(sig_integral)));
		tp_dx->AddText(Form("No Rej efficiency: %.2f%%", 100.0*dx_sig_integral/sig_integral));
		tp_dx->AddText("---------");
		tp_dx->AddText(Form("Estimated Rej. W2 count: %i", int(sig_integral_rej)));
		tp_dx->AddText(Form("Rej efficiency: %.2f%%", 100.0*dx_sig_integral/sig_integral_rej));		
		tp_dx->AddText("---------");
		tp_dx->AddText(Form("Estimated Interpol W2 count: %i", int(h_W2_interpol_sig->Integral(81, 241)) ));
		tp_dx->AddText(Form("Interpolated efficiency: %.2f%%", 100.0*dx_sig_integral/h_W2_interpol_sig->Integral(81, 241)));		
		// tp_dx->Draw("same");

		int W2_interpol_sig_min_bin2 = h_W2_interpol_sig->GetXaxis()->FindBin(0.4);
		int W2_interpol_sig_max_bin2 = h_W2_interpol_sig->GetXaxis()->FindBin(1.2);

		double W2_interpol_sig_cnt2 = h_W2_interpol_sig->Integral(W2_interpol_sig_min_bin, W2_interpol_sig_min_bin);
		cout << "W2_interpol_sig_min_bin2: " << W2_interpol_sig_min_bin2 << endl;
		cout << "W2_interpol_sig_max_bin2: " << W2_interpol_sig_max_bin2 << endl;
		cout << "W2_interpol_sig_cnt2: " << W2_interpol_sig_cnt2 << endl;
		cout << "calc: " << h_W2_interpol_sig->Integral(81, 241) << endl;
