...
double dy_sig_multiplier = 3.5;

double dx_bin_factor = 100.0;
...
double W2_sig_p0, W2_sig_p0_err, W2_sig_p2, W2_sig_p2_err, dx_sig_p0, dx_sig_p0_err, dx_sig_p2, dx_sig_p2_err;
double W2_sig_err, dx_sig_err;

TH1D *h_dx_subrange_sig, *h_dx_sig_from_sub_allVals, *h_bgCut_sig_from_sub_allVals;
...
double dy_sig_norm, dy_sig_mean, dy_sig_sigma, dy_sig_min, dy_sig_max;
...
void det_eff_interpol_binning(){
	...
	outfile_name = Form("rootfiles/det_eff_histos_SBS%i_%s_mag%i_binFactor%i.root", kine, run_target.Data(), sbsfieldscale, int(dx_bin_factor));
...
//-----------------------------------------------
	...
	if( fit_only ){
		...
		tf1_W2_fullFit->SetParLimits(0, 0.5*h_W2->GetMaximum(), h_W2->GetMaximum());
...
//---------------------------------------------
	...
	//----- Then we take the parameters from that and fix the Full Fit BG parametes to match
			if( false ) {
				...
				//----------
					...
					//Fix parameters from the BG_reject fit
						...
						tf1_W2_fullFit_rej->SetParLimits(0, 0.5*h_W2copy->GetMaximum(), h_W2copy->GetMaximum());
		...
		//----------
			...
			//Fix parameters from the BG_reject fit
				...
				h_W2copy->Fit("tf1_W2_fullFit_rej", "R+");
			}

			W2_yield_reject = (tf1_W2_sig_rej->GetParameter(0))*(tf1_W2_sig_rej->GetParameter(2))*sqrt(2*pi)/tf1_W2_sig_rej->GetHistogram()->GetBinWidth(1);
		...
		//------------------------------------------------------------
			...
			TH1D *h_bg_rand_rej = new TH1D("h_bg_rand_rej", "Randomly filled BG from tf for BG with Reject", W2_nbins, 0, 1.4);
	...
	//--------------BGcut fits
		...
		h_rej_sig_gaus = (TH1D*) h_tf_W2_sig->Clone("h_rej_sig_gaus");
		// h_rej_sig_gaus = (TH1D*) h_tf_W2_sig_rej->Clone("h_rej_sig_gaus");
		h_rej_bg = new TH1D("h_rej_bg", "h_rej_bg", W2_nbins, 0, 1.4);
		...
		tf1_W2_bgCut_pol8->SetNpx(W2_nbins);
		...
		tf1_W2_bgCut_full = new TF1("tf1_W2_bgCut_full", fullBGCutFitFunction, W2_min_x, W2_max_x, 12);
		tf1_W2_bgCut_full->SetNpx(W2_nbins);
		...
		bgCut_full_integral = tf1_W2_bgCut_full->Integral(bgCut_gaus_min, bgCut_gaus_max)/tf1_W2_bgCut_full->GetHistogram()->GetBinWidth(1);
		sigma_bgCut_full_integral = tf1_W2_bgCut_full->IntegralError(bgCut_gaus_min, bgCut_gaus_max, nullptr, nullptr, 1.E-8)/tf1_W2_bgCut_full->GetHistogram()->GetBinWidth(1);

		h_W2_bgCut_sig = new TH1D("h_W2_bgCut_sig", "h_W2_bgCut_sig", W2_nbins, W2_min_x, W2_max_x);
		h_W2_bgCut_sig->SetLineColor(6);

		h_bgCut_sig_from_sub_allVals = new TH1D("h_bgCut_sig_from_sub_allVals", "h_bgCut_sig_from_sub_allVals", W2_nbins, W2_min_x, W2_max_x);

		h_W2_bgCut_bg_sub = new TH1D("h_W2_bgCut_bg_sub", "h_W2_bgCut_bg_sub", W2_nbins, W2_min_x, W2_max_x);
		...
		for(int bin = 0; bin <= W2_nbins; bin++ ){
			...
			h_bgCut_sig_from_sub_allVals->SetBinContent(bin, sub_val);
		}

		h_W2_bgCut_sig->Draw("same");

		bgCut_sig_cnt = h_W2_bgCut_sig->Integral();

		cout << "*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--" << endl;
		cout << "--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*" << endl;
		cout << "W2 sig (from sub) cnt: " << bgCut_sig_cnt << endl;
		cout << "*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--" << endl;
		cout << "--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*--*" << endl;

		tf1_W2_bgCut_gaus = new TF1("tf1_W2_bgCut_gaus", fit_gaus, W2_min_x, W2_max_x, 3);
		tf1_W2_bgCut_gaus->SetNpx(W2_nbins);
		...
		TF1 *tf1_bgCut_gaus_err = new TF1("tf1_bgCut_gaus_err", "gaus", W2_min_x, W2_max_x);
		tf1_bgCut_gaus_err->SetNpx(W2_nbins);
		tf1_bgCut_gaus_err->SetLineColor(5);
		tf1_bgCut_gaus_err->SetParLimits(0, 0.99*tf1_W2_bgCut_gaus->GetParameter(0), 1.01*tf1_W2_bgCut_gaus->GetParameter(0));
		tf1_bgCut_gaus_err->SetParLimits(1, 0.99*tf1_W2_bgCut_gaus->GetParameter(1), 1.01*tf1_W2_bgCut_gaus->GetParameter(1));
		tf1_bgCut_gaus_err->SetParLimits(2, 0.99*tf1_W2_bgCut_gaus->GetParameter(2), 1.01*tf1_W2_bgCut_gaus->GetParameter(2));

		h_W2_bgCut_sig->Fit(tf1_bgCut_gaus_err, "RS0");
		// h_bgCut_sig_from_sub_allVals->Fit(tf1_bgCut_gaus_err, "RS0");

		W2_sig_p0 = tf1_bgCut_gaus_err->GetParameter(0);
		W2_sig_p0_err = tf1_bgCut_gaus_err->GetParError(0);

		W2_sig_p2 = tf1_bgCut_gaus_err->GetParameter(2);
		W2_sig_p2_err	= tf1_bgCut_gaus_err->GetParError(2);

		bgCut_sig_integral = tf1_bgCut_gaus_err->Integral(W2_min_x, W2_max_x)/tf1_bgCut_gaus_err->GetHistogram()->GetBinWidth(1);
		sigma_bgCut_sig_integral = tf1_bgCut_gaus_err->IntegralError(W2_min_x, W2_max_x, nullptr, nullptr, 1.E-8)/tf1_bgCut_gaus_err->GetHistogram()->GetBinWidth(1);
		...
		bgCut_sig_gaus_cnt = tf1_W2_bgCut_gaus->Integral(W2_min_x, W2_max_x)/tf1_W2_bgCut_gaus->GetHistogram()->GetBinWidth(1);
...
//----------------------------
	...
	//--------------------------------------
		...
		tf1_W2_interpolate_full->SetNpx( W2_nbins );
		...
		tf1_W2_interpolate_gaus->SetNpx(W2_nbins);
...
//------------------------------------------------------
		...
		if( true ){
			...
			tf1_dy_fullFit = new TF1("tf1_dy_fullFit", fullDYFitFunction, dy_min_y, dy_max_y, 6);
		...
		//Signal Gaussian
			tf1_dy_sig = new TF1("tf1_dy_sig", fit_gaus, dy_min_y, dy_max_y, 3);
			tf1_dy_sig->SetNpx(dy_nbins);
			...
			dy_sig_min = dy_sig_mean - (dy_sig_multiplier)*dy_sig_sigma;
			dy_sig_max = dy_sig_mean + (dy_sig_multiplier)*dy_sig_sigma;
		...
		//BG Pol3
			...
			tf1_dy_bg = new TF1("tf1_dy_bg", fitPol2, dy_min_y, dy_max_y, 3);
			tf1_dy_bg->SetNpx(dy_nbins);
...
//------------------------------------------------------
		...
		h_dxdy->GetYaxis()->SetRangeUser(dx_min_x, dx_max_x);
		...
		tf1_dx_fullFit = new TF1("tf1_dx_fullFit", fullDXFitFunction, dx_min_x, dx_max_x, 7);
		...
		dx_full_integral = tf1_dx_fullFit->Integral(dx_min_x, dx_max_x)/tf1_dx_fullFit->GetHistogram()->GetBinWidth(1);
		sigma_dx_full_integral = tf1_dx_fullFit->IntegralError(dx_min_x, dx_max_x, nullptr, nullptr, 1.E-8)/tf1_dx_fullFit->GetHistogram()->GetBinWidth(1);
		
		tf1_dx_fullFit->GetParameters(&par[0]);		

		tf1_dx_sig = new TF1("tf1_dx_sig", fit_gaus, dx_min_x, dx_max_x, 3);
		tf1_dx_sig->SetNpx(dx_nbins);
		tf1_dx_sig->SetParLimits(0, 0.99*par[0], 1.01*par[0]);
		tf1_dx_sig->SetParLimits(1, 0.99*par[1], 1.01*par[1]);
		tf1_dx_sig->SetParLimits(2, 0.99*par[2], 1.01*par[2]);

		h_dx_subrange->Fit(tf1_dx_sig, "S");

		dx_sig_p0 = tf1_dx_sig->GetParameter(0);
		dx_sig_p0_err = tf1_dx_sig->GetParError(0);

		dx_sig_p2 = tf1_dx_sig->GetParameter(2);
		dx_sig_p2_err = tf1_dx_sig->GetParError(2);
		...
		tf1_dx_bg = new TF1("tf1_dx_bg", fitPol3, dx_min_x, dx_max_x, 4);
		tf1_dx_bg->SetNpx(dx_nbins);
		...
		TF1 *tf1_dx_bg_pol3_err = new TF1("tf1_dx_bg_pol3_err", "pol3", dx_min_x, dx_max_x);
		tf1_dx_bg_pol3_err->SetNpx(dx_nbins);
		...
		dx_bg_integral = tf1_dx_bg_pol3_err->Integral(dx_min_x, dx_max_x);

		sigma_dx_bg_integral = tf1_dx_bg_pol3_err->IntegralError(dx_min_x, dx_max_x, nullptr, nullptr, 1.E-8);
		...
		h_dx_sig = new TH1D("h_dx_sig", "h_dx_sig", dx_nbins, dx_min_x, dx_max_x);
		...
		h_dx_subrange_sig = new TH1D("h_dx_subrange_sig", "h_dx_subrange_sig", dx_nbins, dx_min_x, dx_max_x);
		h_dx_sig_from_sub_allVals = new TH1D("h_dx_sig_from_sub_allVals", "h_dx_sig_from_sub_allVals", dx_nbins, dx_min_x, dx_max_x);
		...
		for( int bin = 0; bin <= dx_nbins; bin++ ){
			...
			h_dx_sig_from_sub_allVals->SetBinContent(bin, sub_val);
		}


		TF1 *tf1_dx_sig_err = new TF1("tf1_dx_sig_err", "gaus", dx_min_x, dx_max_x);
		tf1_dx_sig_err->SetNpx(dx_nbins);
		// tf1_dx_sig_err->SetNpx(int(200.0*(dx_sig_gaus_max - dx_sig_gaus_min) ));

		h_dx_subrange_sig->Fit(tf1_dx_sig_err, "RS0+");
		// h_dx_sig_from_sub_allVals->Fit(tf1_dx_sig_err, "RS0+");

		dx_sig_integral = tf1_dx_sig_err->Integral(dx_min_x, dx_max_x)/tf1_dx_sig_err->GetHistogram()->GetBinWidth(1);
		sigma_dx_sig_integral = tf1_dx_sig_err->IntegralError(dx_min_x, dx_max_x, nullptr, nullptr, 1.E-8)/tf1_dx_sig_err->GetHistogram()->GetBinWidth(1);

		sigma_dx_TOTAL = sqrt( pow( sigma_dx_full_integral, 2) + pow( sigma_dx_sig_integral, 2) );


		dx_sig_integral = h_dx_sig->Integral();
	...
	// 	sig_integral = h_dx_sig->Integral();
		...
		double Sig_dx = 100.0*sigma_dx_sig_integral/dx_sig_integral;
		double Sig_w2 = 100.*sigma_bgCut_sig_integral/bgCut_sig_integral;

		double Sig = 100.0*det_eff_FINAL*sqrt( pow(Sig_dx, 2) + pow(Sig_w2, 2) );

		cout << "W2_nbins: " << W2_nbins << endl;
		cout << "dx_nbins: " << dx_nbins << endl;
		cout << "dy_sig_min = " << dy_sig_min << ", dy_sig_max = " << dy_sig_max << endl;
		cout << "Sigma from invidual IntegralErrors: " << Sig << endl;
	...
	if( fit_only ){
		...
		cout << "-------------------------------" << endl;
		cout << "---- Binning Systematics ------" << endl;
		cout << "-------------------------------" << endl;

		cout << "dy_sig_norm: " << dy_sig_norm << endl;
		cout << "dy_sig_mean: " << dy_sig_mean << endl;
		cout << "dy_sig_sigma: " << dy_sig_sigma << endl;
		cout << "dy_sig_multiplier: " << dy_sig_multiplier << endl;
		cout << "dy_sig_min: " << dy_sig_min << endl;
		cout << "dy_sig_max: " << dy_sig_max << endl;
		cout << endl;
		cout << "dx_min_x: " << dx_min_x << endl;
		cout << "dx_max_x: " << dx_max_x << endl;
		cout << "dx_bin_factor: " << dx_bin_factor << endl;
		cout << "dx_nbins: " << dx_nbins << endl;
		cout << endl << endl;

		cout << "Detector efficiency 1 = " << 100.0*det_eff_FINAL << "% +/- " << basic_TOTAL_sigma << "%" << endl;
		cout << endl;
		cout << "Detector efficiency 2 = " << 100.0*det_eff_FINAL << "% +/- " << 100.0*det_eff_FINAL*sqrt( pow(Sig_dx, 2) + pow(Sig_w2, 2) ) << "%" << endl;
