...
TH1D *h_W2elastics, *h_W2elastics_fcut, *h_W2elastics_init, *h_W2elastics_gaus, *h_W2elastics_corr;
TH1D *h_dy_sig;
...
double theta_pq_p_thresh = 0.020;
double theta_pq_n_thresh = 0.020;
...
double p_el, nu, pp, nucleon_theta, nucleon_phi, E_ep, p_ep, pN, Q2, W, W2, E_pp, E_nucleon, KE_p, dx, dy;
...
double p_recon, nu_recon, E_loss, E_corr, theta_pq_n, theta_pq_p, theta_pq_n_pNhat, theta_pq_p_pNhat;
...
TF1 *tf1_dx_fullFit, *tf1_dx_bg_pol8, *tf1_dx_sig, *tf1_W2_bg;
...
void det_eff_interpol(){
	...
	//-------------
			...
			pNhat( sin(nucleon_theta)*cos(nucleon_phi), sin(nucleon_theta)*sin(nucleon_phi), cos(nucleon_theta) );
	...
	//Calculate theta pq variables
		...
		//Expected proton direction
			...
			theta_pq_n_pNhat = acos( Neutron_Direction.Dot( pNhat ) );
			theta_pq_p_pNhat = acos( Proton_Direction.Dot( pNhat ) );

			h_theta_pq_n_pNhat->Fill(theta_pq_n_pNhat);
			h_theta_pq_p_pNhat->Fill(theta_pq_p_pNhat);

		//SECONDARY METHOD FOR THETA_PQ
			TVector3 protdir = ( hcalpos + protdeflect * hcalaxes[0] - vertex ).Unit();
			thetapq_p = acos( protdir.Dot( pNhat ));
			thetapq_n = acos( neutdir.Dot( pNhat ));
...
//-----------------------------------------------
	...
	if( fit_only ){
		...
		h_W2interpol = (TH1D*)h_W2->Clone("h_W2intepol");
...
//---------------------------------------------
	...
	//--------------------------------------
		...
		h_W2interpol->Fit("tf1_W2_interpolate", "RBM+");
		...
		h_W2interpol->Fit("tf1_W2_interpol_bg", "RBM+");
...
//------------------------------------------------------
		...
		tf1_dx_fullFit = new TF1("tf1_dx_fullFit", fullDXFitFunction, -2.5, 2.5, 7);
		...
		double dx_sig_integral = h_tf_dx_sig->Integral();
